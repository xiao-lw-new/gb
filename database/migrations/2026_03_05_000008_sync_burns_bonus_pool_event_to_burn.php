...
use App\Modules\Blockchain\Handlers\EventToBurnHandler;
...
return new class extends Migration
{
    ...
    {
        $abiPath = base_path('build/BurnsBonusPool.json');
        ...
        $contracts = DB::table('blockchain_contract')->where('name', 'BurnsBonusPool')->get();
        if ($contracts->isEmpty()) {
            return;
        }

        $signature = $this->buildEventSignature($abi, 'EventToBurn');
        if (!$signature) {
            return;
        }
        ...
        foreach ($contracts as $contract) {
            DB::table('blockchain_contract_event')->updateOrInsert(
                [
                    ...
                    'event_name' => 'EventToBurn',
                ],
                [
                    'handler' => EventToBurnHandler::class,
                    'topic' => $topic,
                    'remark' => $signature,
                    'status' => 1,
                    'updated_at' => now(),
